...
export default function LoadingIcon({time = 1, tileSize = 1, color, className} : {time?: number, tileSize?: number, color: string, className?: string}) {
    ...
    return (
        <svg ref={svg} viewBox={`${-size} ${-size} ${size*4} ${size*4}`} xmlns="http://www.w3.org/2000/svg"  className={`animate__animated animate__fadeIn animate__slower ${className}`} width={tileSize} height={tileSize}>
        <defs/>
